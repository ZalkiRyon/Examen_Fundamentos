...
def calcular_estadisticas(sueldos):
    estadisticas = {}
    estadisticas['Promedio'] = statistics.mean(sueldos)
    estadisticas['Mediana'] = statistics.median(sueldos)    
    estadisticas['Sueldo Máximo'] = max(sueldos)
    estadisticas['Sueldo Mínimo'] = min(sueldos)    
    clasificacion = clasificar_sueldos(sueldos)
    for rango, lista in clasificacion.items():
        if lista:
            promedio = statistics.mean(lista)
            estadisticas[f'Promedio {rango}'] = promedio
    return estadisticas
